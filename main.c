...
static struct slurp_output *output_from_surface(struct slurp_state *state, struct wl_surface *surface);

static void move_seat(struct slurp_seat *seat, wl_fixed_t surface_x, wl_fixed_t surface_y, struct slurp_selection *current_selection) {
...
static void seat_update_selection(struct slurp_seat *seat) {
    ...
    wl_list_for_each(box, &seat->state->boxes, link) {
        if (in_box(box, seat->pointer_selection.x, seat->pointer_selection.y)) {
            if (seat->pointer_selection.has_selection && box_size(&seat->pointer_selection.selection) < box_size(box)) {
                continue;
            }
...
static void seat_set_outputs_dirty(struct slurp_seat *seat) {
    ...
    wl_list_for_each(output, &seat->state->outputs, link) {
        ...
        if (box_intersect(geometry, &seat->pointer_selection.selection) || box_intersect(geometry, &seat->touch_selection.selection) || (state->crosshairs && in_box(geometry, seat->pointer_selection.x, seat->pointer_selection.y))) {
...
static void pointer_handle_enter(void *data, struct wl_pointer *wl_pointer, uint32_t serial, struct wl_surface *surface, wl_fixed_t surface_x, wl_fixed_t surface_y) {
    ...
    if (output->state->cursor_shape_manager) {
        struct wp_cursor_shape_device_v1 *device = wp_cursor_shape_manager_v1_get_pointer(output->state->cursor_shape_manager, wl_pointer);
        ...
        wl_surface_attach(seat->cursor_surface, wl_cursor_image_get_buffer(output->cursor_image), 0, 0);
        wl_pointer_set_cursor(wl_pointer, serial, seat->cursor_surface, output->cursor_image->hotspot_x / output->scale, output->cursor_image->hotspot_y / output->scale);
...
static void handle_selection_start(struct slurp_seat *seat, struct slurp_selection *current_selection) {
    struct slurp_state *state = seat->state;
...
static void handle_selection_end(struct slurp_seat *seat, struct slurp_selection *current_selection) {
    struct slurp_state *state = seat->state;
...
static void keyboard_handle_keymap(void *data, struct wl_keyboard *wl_keyboard, const uint32_t format, const int32_t fd, const uint32_t size) {
    struct slurp_seat *seat = data;
    switch (format) {
...
static void keyboard_handle_key(void *data, struct wl_keyboard *wl_keyboard, const uint32_t serial, const uint32_t time, const uint32_t key, const uint32_t key_state) {
    struct slurp_seat  *seat   = data;
    struct slurp_state *state  = seat->state;
    ...
    case WL_KEYBOARD_KEY_STATE_PRESSED:
        ...
        case XKB_KEY_space:
            if (!seat->pointer_selection.has_selection && !seat->touch_selection.has_selection) {
                break;
            }
...
static void keyboard_handle_modifiers(void *data, struct wl_keyboard *wl_keyboard, const uint32_t serial, const uint32_t mods_depressed, const uint32_t mods_latched, const uint32_t mods_locked, const uint32_t group) {
    struct slurp_seat *seat = data;
...
static void touch_handle_down(void *data, struct wl_touch *touch, uint32_t serial, uint32_t time, struct wl_surface *surface, int32_t id, wl_fixed_t x, wl_fixed_t y) {
    ...
    if (seat->touch_id == TOUCH_ID_EMPTY) {
        seat->touch_id                       = id;
        seat->touch_selection.current_output = output_from_surface(seat->state, surface);
...
static void create_output(struct slurp_state *state, struct wl_output *wl_output) {
...
static void send_frame(struct slurp_output *output) {
    ...
    output->current_buffer = get_next_buffer(state->shm, output->buffers, buffer_width, buffer_height);
...
static void output_frame_handle_done(void *data, struct wl_callback *callback, uint32_t time) {
    struct slurp_output *output = data;
...
static void handle_global(void *data, struct wl_registry *registry, uint32_t name, const char *interface, uint32_t version) {
    ...
    if (strcmp(interface, wl_compositor_interface.name) == 0) {
        ...
        struct wl_seat *wl_seat = wl_registry_bind(registry, name, &wl_seat_interface, 1);
        create_seat(state, wl_seat);
    } else if (strcmp(interface, wl_output_interface.name) == 0) {
        struct wl_output *wl_output = wl_registry_bind(registry, name, &wl_output_interface, 3);
        ...
        state->cursor_shape_manager = wl_registry_bind(registry, name, &wp_cursor_shape_manager_v1_interface, 1);
    }
}
...
static const char usage[] = "Usage: slurp [options...]\n"
                            "\n"
...
static void add_choice_box(struct slurp_state *state, const struct slurp_box *box) {
...
static bool create_cursors(struct slurp_state *state) {
    ...
    struct slurp_output *output;
    wl_list_for_each(output, &state->outputs, link) {
        output->cursor_theme = wl_cursor_theme_load(cursor_theme, cursor_size * output->scale, state->shm);
        if (output->cursor_theme == NULL) {
            fprintf(stderr, "failed to load cursor theme\n");
            return false;
        }
        struct wl_cursor *cursor = wl_cursor_theme_get_cursor(output->cursor_theme, "crosshair");
        if (cursor == NULL) {
            // Fallback
            cursor = wl_cursor_theme_get_cursor(output->cursor_theme, "left_ptr");
        }
        if (cursor == NULL) {
...
int slurp_main(int argc, char *argv[]) {
    ...
    wl_list_for_each(output, &state.outputs, link) {
        ...
        output->layer_surface = zwlr_layer_shell_v1_get_layer_surface(state.layer_shell, output->surface, output->wl_output, ZWLR_LAYER_SHELL_V1_LAYER_OVERLAY, "selection");
        ...
        zwlr_layer_surface_v1_set_anchor(output->layer_surface, ZWLR_LAYER_SURFACE_V1_ANCHOR_TOP | ZWLR_LAYER_SURFACE_V1_ANCHOR_LEFT | ZWLR_LAYER_SURFACE_V1_ANCHOR_RIGHT | ZWLR_LAYER_SURFACE_V1_ANCHOR_BOTTOM);
